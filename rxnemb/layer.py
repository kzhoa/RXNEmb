...
from .utils import index_select_ND, scaled_dot_product_attention
...
class DGCNEncoder(nn.Module):
    ...
    def _build_layers(self) -> None:
        ...
        self.W_o = nn.Sequential(
            nn.Linear(self.node_fdim + self.h_size, self.h_size), nn.GELU()
        )
...
class DGATGRU(nn.Module):
    ...
    def __init__(
        self, enc_attn_heads, input_size, h_size, depth, drop_ratio, att_drop_ratio
    ):
        super().__init__()
        self.enc_attn_heads = enc_attn_heads
        self.input_size = input_size
    ...
    def _build_attention(self) -> None:
        ...
        self.attn_alpha = nn.Parameter(
            torch.Tensor(1, 1, self.head_count, 2 * self.dim_per_head),
            requires_grad=True,
        )
    ...
    def GRU(self, x: torch.Tensor, h_nei: torch.Tensor) -> torch.Tensor:
        ...
        q = q.reshape(
            n_node, max_nn, head_count, dim_per_head
        )  # -> (n_node, max_nn, head, h/head)
        ...
        attn_score = torch.sum(
            attn_score, dim=-1
        )  # (n_node, max_nn, head, 2*h/head) -> (n_node, max_nn, head)
        attn_score = attn_score + self.attn_bias  # (n_node, max_nn, head)

        attn_mask = (h_nei.sum(dim=2) == 0).unsqueeze(
            2
        )  # (n_node, max_nn, h) -> (n_node, max_nn, 1)
        attn_score = attn_score.masked_fill(attn_mask, -1e18)

        attn_weight = self.softmax(
            attn_score
        )  # (n_node, max_nn, head), softmax over dim=1
...
class DGATEncoder(nn.Module):
    ...
    def forward(self, fnode, fmess, agraph, bgraph, mask):
        ...
        q = q.reshape(
            n_node, max_nn, head_count, dim_per_head
        )  # (n_node, max_nn, h) -> (n_node, max_nn, head, h/head)
        ...
        attn_mask = (nei_message.sum(dim=2) == 0).unsqueeze(
            2
        )  # (n_node, max_nn, h) -> (n_node, max_nn, 1)
        attn_score = attn_score.masked_fill(attn_mask, -1e18)
