...
def get_random_shuffle_smiles(
    init_smi,
    rxn_template="[C,N,O:1]-[*:2]~[*:3]-[*:4]>>[*:4]-[*:2]~[*:3]-[C,N,O:1]",
    sel_num=2,
    random_seed=42,
):
    """
    ...
    if len(rxn_res) > 0:
        ...
        sel_idx = np.random.choice(
            len(rxn_smi_res), min(sel_num, len(rxn_smi_res)), replace=False
        )
...
def gen_truth_false_rxn_smi(
    ...
    false_pdt_smi_lst = get_random_shuffle_smiles(
        pdt_smi, rxn_template=rxn_template, sel_num=sel_num, random_seed=random_seed
    )
    if not split:
...
def get_sin_encodings(rel_pos_buckets, model_dim):
    ...
    )  # leaving last "position" as padding
...
def mod_mol(
    mol1, mol2=None, type="add", bond_idx=[0, 1], bond_type=Chem.BondType.SINGLE
):
    type = type.lower()
    if type == "add":
...
def gen_mid_mols(rct_blks, pdt_blks):
    ...
    products = [
        reaction.GetProductTemplate(i) for i in range(reaction.GetNumProductTemplates())
    ]
    ...
    for rct_idx, reactant in enumerate(reactants):
        ...
        rct_mol_atom_idx_map.update(
            {atom.GetAtomMapNum(): [rct_idx, atom.GetIdx()] for atom in atoms}
        )
    ...
    for pdt_idx, product in enumerate(products):
        ...
        pdt_mol_atom_idx_map.update(
            {atom.GetAtomMapNum(): [pdt_idx, atom.GetIdx()] for atom in atoms}
        )
    ...
    for bond in new_bonds_in_pdt:
        try:
            mid_mol = mod_mol(
                pdt_mols[bond[0][0]], None, "remove", [bond[0][1], bond[1][1]]
            )
        except:
            continue
        all_mid_mols.append(mid_mol)
...
def add_empty_node_and_edge(batch_data):
    ...
    oh_empty_node = (
        torch.from_numpy(gen_onehot(empty_node, ATOM_FEAT_DIMS))
        .unsqueeze(0)
        .long()
        .to(batch_data.x_oh.device)
    )
    oh_empty_edge = (
        torch.zeros(1, batch_data.edge_oh_attr.shape[1])
        .long()
        .to(batch_data.x_oh.device)
    )
    a_graphs_empty = (
        torch.zeros(1, batch_data.a_graphs.shape[1]).long().to(batch_data.x_oh.device)
    )
    b_graphs_empty = (
        torch.zeros(1, batch_data.b_graphs.shape[1]).long().to(batch_data.x_oh.device)
    )
...
def add_dense_empty_node_edge(batch_data):
    ...
    empty_edge = (
        torch.zeros(1, batch_data.edge_attr.shape[1]).long().to(batch_data.x.device)
    )
    x_merge_ = torch.cat([empty_node, batch_data.x], dim=0)
    edge_merge_ = torch.cat([empty_edge, batch_data.edge_attr], dim=0)
    edge_index_ = torch.cat(
        [torch.zeros(2, 1).long().to(batch_data.x.device), batch_data.edge_index + 1],
        dim=1,
    )
