...
def calc_batch_graph_distance(batch, edge_index, task):
    ...
    full_adj_matrix = torch.zeros(
        num_nodes, num_nodes, dtype=torch.int32, device=batch.device
    )
    src = edge_index[0]
    dest = edge_index[1]
    ...
    for i in range(num_graphs):
        ...
        max_power = max_len
        identity = torch.eye(
            adj_matrix.size(0), dtype=torch.int32, device=adj_matrix.device
        )
        ...
        for idx, raw_data_f in enumerate(self.raw_data_files):
            ...
            for rxn_smi_tgt in tqdm(rxn_smi_tgt_lst):
                ...
                data = Data(
                    ...
                    mol_index=mol_index,
                    y=tgt_,
                    ext_feat=torch.tensor(ext_feat_desc).float(),
                )
        ...
        if not self.ext_feat:
            ...
            if self.ext_feat_type == "rdkit":
                if not self.tag:
                    ...
                    return [
                        f"{self.name[:-4]}_{self.trunck}_{self.ext_feat_type}_{self.tag}.pt"
                    ]
            elif self.ext_feat_type in ["morgan", "atompair", "toptorsion", "rdfp"]:
                if not self.tag:
                    return [
                        f'{self.name[:-4]}_{self.trunck}_{self.ext_feat_type}_{self.ext_feat_param["radius"]}_{self.ext_feat_param["nBits"]}.pt'
                    ]
                else:
                    return [
                        f'{self.name[:-4]}_{self.trunck}_{self.ext_feat_type}_{self.ext_feat_param["radius"]}_{self.ext_feat_param["nBits"]}_{self.tag}.pt'
                    ]
            elif "++" in self.ext_feat_type:
                if not self.tag:
                    return [
                        f"{self.name[:-4]}_{self.trunck}_{self.ext_feat_type}_{self.mul_ext_readout}.pt"
                    ]
                else:
                    return [
                        f"{self.name[:-4]}_{self.trunck}_{self.ext_feat_type}_{self.mul_ext_readout}_{self.tag}.pt"
                    ]
            else:
        ...
        if self.multi_process:
            ...
            for i in tqdm(range(0, len(rxn_smi_tgt_lst), self.batch_size)):
                batch = (
                    ...
                    for j in range(i, min(i + self.batch_size, len(rxn_smi_tgt_lst)))
                )
...
def get_train_val_test_token_data(
    src, tgt, val_ratio=0.1, test_ratio=0.1, random_seed=42
):
...
def generate_regression_dataset(
    dataset_file,
    rct_cols=["Reactant1", "Reactant2", "Solvents", "Reagents"],
    pdt_cols=["Product"],
    tgt_inf=None,
):
...
def gen_mol_in_rxn(rxn_smiles, show_atom_map=True):
    ...
    for smi in smi_blk_lst:
        ...
        if show_atom_map:
            [
                atom.SetAtomMapNum(atom.GetIdx() + 1) for atom in rdkit_mol.GetAtoms()
            ]  ## atom map number have to start from 1
        if rdkit_mol == None:
            failed = True
            break
