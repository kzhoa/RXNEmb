    ...
    def __init__(
        ...
        self.test_ground_truth_smiles_lst = [
            "".join(
                [
                    self.vocab_rev[idx]
                    for idx in test_dataset[idx].tgt_token_ids[0][
                        : int(test_dataset[idx].tgt_lens[0]) - 1
                    ]
                ]
            )
...
def eval_regression_performance(
    ...
    if not specific_val:
        rct_dataset = RXNDataset(
            root=pretrained_config.data.data_path,
            name=pretrained_config.data.rct_data_file,
            trunck=pretrained_config.data.data_trunck,
        )
        pdt_dataset = RXNDataset(
            root=pretrained_config.data.data_path,
            name=pretrained_config.data.pdt_data_file,
            trunck=pretrained_config.data.data_trunck,
        )
        mid_dataset = RXNDataset(
            root=pretrained_config.data.data_path,
            name=pretrained_config.data.mid_data_file,
            trunck=pretrained_config.data.data_trunck,
        )

        split_ids_map = get_idx_split(
            len(rct_dataset),
            int(pretrained_config.data.train_ratio * len(rct_dataset)),
        ...
        test_dataset = TripleDataset(
            test_rct_dataset, test_pdt_dataset, test_mid_dataset
        )
        test_dataloader = torch.utils.data.DataLoader(
            test_dataset,
            batch_size=pretrained_config.data.batch_size,
            shuffle=False,
        ...
        if return_train_results:
            ...
            train_dataset = TripleDataset(
                train_rct_dataset, train_pdt_dataset, train_mid_dataset
            )
        ...
        test_rct_dataset = RXNDataset(
            root=pretrained_config.data.data_path,
            name=pretrained_config.data.test_rct_data_file,
            trunck=pretrained_config.data.data_trunck,
        )
        test_pdt_dataset = RXNDataset(
            root=pretrained_config.data.data_path,
            name=pretrained_config.data.test_pdt_data_file,
            trunck=pretrained_config.data.data_trunck,
        )
        test_mid_dataset = RXNDataset(
            root=pretrained_config.data.data_path,
            name=pretrained_config.data.test_mid_data_file,
            trunck=pretrained_config.data.data_trunck,
        )
        test_dataset = TripleDataset(
            test_rct_dataset, test_pdt_dataset, test_mid_dataset
        ...
        if return_train_results:
            train_rct_dataset = RXNDataset(
                root=pretrained_config.data.data_path,
                name=pretrained_config.data.train_rct_data_file,
                trunck=pretrained_config.data.data_trunck,
            )
            train_pdt_dataset = RXNDataset(
                root=pretrained_config.data.data_path,
                name=pretrained_config.data.train_pdt_data_file,
                trunck=pretrained_config.data.data_trunck,
            )
            train_mid_dataset = RXNDataset(
                root=pretrained_config.data.data_path,
                name=pretrained_config.data.train_mid_data_file,
                trunck=pretrained_config.data.data_trunck,
            )
            train_dataset = TripleDataset(
                train_rct_dataset, train_pdt_dataset, train_mid_dataset
        ...
        with torch.no_grad():
            for step, batch_data in enumerate(train_dataloader):
                ...
                train_targets = torch.cat(
                    [train_targets, rct_data.y.unsqueeze(1)], dim=0
                )
...
def load_pred_model(
    pretrained_model_path, ckpt_filename="valid_checkpoint.pt", task_type="reactivity"
):
    assert task_type in [
        "reactivity",
        "selectivity",
...
def reaction_prediction(
    ...
    if task_type in ["reactivity", "selectivity"]:
        ...
        with open(
            "./rxn_emb_tmp/tgt_tokenized_smiles.txt", "w"
        ) as fw:  # just a placeholder
            fw.writelines("\n".join(tokenized_smiles_lst))
        ...
        shutil.copyfile(
            f"{pretrained_config.data.data_path}/{pretrained_config.data.vocab_file}",
            f"./rxn_emb_tmp/{pretrained_config.data.vocab_file}",
        )
        eval_dataloader = get_eval_dataloader(
            "./rxn_emb_tmp",
